...
--     end
--
--     -- cancel the completion if it's active
--     local function cancel_completion(fallback)
--         if cmp.visible() then
--             cmp.close()
--         else
--             fallback()
...
--             ["<Tab>"] = cmp.mapping(confirm_cmp_or_jump, { "i", "s" }), -- TODO this may need tweaking, currently there's no hotkey for jumping but not confirming
--             ["<S-Tab>"] = cmp.mapping(jump_backwards, { "i", "s" }),
--
--             -- ^c to cancel the completion without entering normal mode
--             ["<C-c>"] = cmp.mapping(cancel_completion, { "i", "s" }),
--         }),
--         completion = {
